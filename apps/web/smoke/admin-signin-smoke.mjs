...
async function runScenario(page, baseUrl, signedIn) {
  const pageErrors = [];
...
try {
  await runScenario(await browser.newPage(), baseUrl, false);
  await runScenario(await browser.newPage(), baseUrl, true);
  console.log(`Brain admin sign-in smoke passed at ${baseUrl}/admin`);
} finally {
  await browser.close();
