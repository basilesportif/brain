#!/usr/bin/env node
// Playwright smoke for the React `/admin-v2` console (plan §8 step 4).
//
...
//   3. an unbuilt console returns the 503 "not built" page.
//
// AUTH LIMITATION (documented): the `/admin-v2` shell is served unauthenticated
...
const ROUTES = ["/admin-v2", "/admin-v2/setup", "/admin-v2/settings", "/admin-v2/users", "/admin-v2/operations"];
...
function makeConfig(root, adminV2Dir) {
...
async function main() {
  ...
  const builtRoot = await mkdtemp(path.join(tmpdir(), "brain-adminv2-smoke-"));
  const unbuiltRoot = await mkdtemp(path.join(tmpdir(), "brain-adminv2-unbuilt-"));
  const emptyDist = path.join(unbuiltRoot, "empty-dist");
  await mkdir(emptyDist, { recursive: true });

  const builtServer = createBrainAdminServer(makeConfig(builtRoot, distDir), {});
  const unbuiltServer = createBrainAdminServer(makeConfig(unbuiltRoot, emptyDist), {});
  ...
  try {
    ...
    // 3. Unbuilt console returns the 503 "not built" page (no path leak).
    const unbuilt = await fetch(`${unbuiltUrl}/admin-v2`);
    ...
    // 4. Browser: the shell mounts (see AUTH LIMITATION above).
    ...
    await page.goto(`${baseUrl}/admin-v2`, { waitUntil: "domcontentloaded" });
    ...
    console.log(`admin-v2 smoke passed: shell + 5 routes + 503 at ${baseUrl}/admin-v2`);
  } finally {
    await browser.close();
