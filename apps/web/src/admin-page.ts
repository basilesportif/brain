...
function nav(): string {
  const items = [
    ["Overview", "overview"],
    ["Slack Setup / Health", "slack-setup"],
...
function dashboard(): string {
  return h("div", { class: "content" },
    ...
    h("section", { id: "mode-panel", class: "mode-panel info", "aria-labelledby": "mode-title" },
      h("div", {},
        h("div", { class: "mode-kicker" }, "State-aware operator console"),
        h("h2", { id: "mode-title" }, "Loading Brain state…"),
        h("p", { id: "mode-description", class: "muted" }, "Checking Slack setup and mission-control readiness."),
      ),
      h("div", {},
    ...
    section("slack-setup", "Slack Setup / Health", "Guided Slack checklist shown by default until Slack is complete, with diagnostics still available below.",
      h("div", { class: "two-col" },
        h("div", { class: "card" },
          h("h3", {}, "Slack setup wizard"),
...
].join('');document.getElementById('slack-wizard').innerHTML=html;document.getElementById('slack-next-action').innerHTML=p.present===p.required?alertHtml('ok','Slack setup appears complete','Required settings are present. Mission Control is the default view; live event verification remains diagnostic/manual until telemetry is added.'):alertHtml('warn',p.present===0?'Slack setup required':'Slack partially configured','Add missing required settings: '+missing+'. Skip is temporary and resets on reload.');}
function updateStateAwareMode(slack,settings,health){if(!slack)return;const state=deriveSlackState(slack);const p=slackPresence(slack);const setupDefault=(state==='not_configured'||state==='partial')&&!skippedSlackForSession;const panel=document.getElementById('mode-panel');panel.className='mode-panel '+(state==='verified'?'ok':setupDefault?'warn':'info');document.getElementById('mode-title').textContent=setupDefault?slackStateLabel(state):'Mission Control';document.getElementById('mode-description').textContent=setupDefault?'To receive Slack events, finish the guided Slack setup wizard. Operator diagnostics and controls remain available below.':(state==='verified'?'Slack required settings are present. Mission Control is the default operator view.':'Slack setup was skipped for this page session. Mission Control is available now; reload returns to Slack setup if still incomplete.');document.getElementById('mode-facts').innerHTML='<div class="mode-fact"><strong>Current Slack state</strong><div class="small muted">'+esc(state)+(skippedSlackForSession?' · skipped_for_session':'')+'</div></div><div class="mode-fact"><strong>Required settings</strong><div class="small muted">'+p.present+'/'+p.required+' present'+(p.missingKeys.length?' · missing '+esc(p.missingKeys.join(', ')):'')+'</div></div><div class="mode-fact"><strong>Target</strong><div class="small muted">'+esc(settings?.codexChat?.serviceName||'codex-chat.service')+' · '+esc(settings?.codexChat?.host||health?.hostname||'local')+'</div></div>';document.getElementById('mode-actions').innerHTML=setupDefault?'<button class="btn primary" type="button" onclick="continueSlackSetup()">Continue Slack setup</button><button class="btn ghost" type="button" onclick="skipSlackForNow()">Skip Slack for now</button><a class="btn ghost" href="#advanced">View diagnostics</a>':'<button class="btn primary" type="button" onclick="openMissionControl()">Open Mission Control</button><a class="btn ghost" href="#slack-setup">Open Slack setup</a><a class="btn ghost" href="#audit">Recent activity</a>';document.getElementById('slack-setup').classList.toggle('mode-hidden',!setupDefault&&state==='verified');document.getElementById('mission').classList.toggle('mode-hidden',setupDefault);}
