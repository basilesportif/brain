...
export function renderBrainAdminPage(config: BrainAdminServiceConfig, adminEmail: string): string {
  ...
  <section id="status" class="card"><h2>Loading…</h2><p class="muted">Checking server-side admin allowlist for <span class="mono">${escapeHtml(adminEmail || "current Clerk account")}</span>.</p><div class="row"><button class="secondary" onclick="signOut()">Sign out / switch account</button></div></section>
  <div class="grid">
    ...
    <section class="card"><h2>Slack settings</h2><p class="muted">Explicit write-only controls for codex-chat Slack env. Responses show presence only. Public Events URL is <span class="mono">https://brain.decisive-outcomes.com/api/slack/events</span>.</p><div id="slack-fields"></div><label>Approval</label><input id="slack-approval" placeholder="write Slack settings"><button onclick="writeSlackSettings()">Write Slack settings</button><pre id="slack-result" class="mono"></pre></section>
    <section class="card"><h2>Slack manifest</h2><p class="muted">Rendered by codex-chat's checked-in manifest script so Brain does not fork Slack app semantics.</p><div class="row"><button onclick="loadManifest()">Render</button><button class="secondary" onclick="copyManifest()">Copy</button><button class="secondary" onclick="downloadManifest()">Download</button></div><p id="manifest-url" class="mono"></p><textarea id="manifest-text" class="mono manifest" readonly></textarea><pre id="manifest-result" class="mono"></pre></section>
...
function renderSlackFields(settings){const present=new Map((settings?.env?.keys||[]).map(k=>[k.key,k.present]));document.getElementById('slack-fields').innerHTML=SLACK_KEYS.map(k=>'<label>'+k+' <span class="muted">'+(present.get(k)?'present':'not set')+'</span></label><input id="slack-'+k+'" type="password" placeholder="leave blank to keep existing value">').join('')}
async function refresh(){try{const me=await api('/me');const email=me.email||CONFIG.adminEmail||'allowlisted admin';document.getElementById('account-email').textContent=email;document.getElementById('status').innerHTML='<h2 class="ok">Signed in and allowlisted</h2><p class="muted">Current Clerk account: <span class="mono">'+email+'</span></p><div class="row"><button class="secondary" onclick="signOut()">Sign out / switch account</button></div>';show('health',await api('/health'));const settings=await api('/settings');show('settings',settings);renderSlackFields(settings.slack||await api('/slack/settings'))}catch(e){const email=e.payload?.email||CONFIG.adminEmail||document.getElementById('account-email').textContent||'current Clerk account';document.getElementById('status').innerHTML='<h2 class="bad">Access problem</h2><p>Server response: '+(e.payload?.error||e.message)+'</p><p class="muted">Current Clerk account: <span class="mono">'+email+'</span></p><div class="row"><button class="secondary" onclick="signOut()">Sign out / switch account</button><a class="button secondary" href="'+CONFIG.signInUrl+'">Account page</a></div>'}}
async function writeEnv(){try{show('env-result',await api('/codex-chat/env',{method:'POST',body:JSON.stringify({entries:{[document.getElementById('env-key').value]:document.getElementById('env-value').value},approval:document.getElementById('env-approval').value})}));document.getElementById('env-value').value='';await refresh()}catch(e){show('env-result',e.payload||e.message)}}
async function writeSlackSettings(){try{const entries={};for(const k of SLACK_KEYS){const v=document.getElementById('slack-'+k).value;if(v)entries[k]=v}show('slack-result',await api('/slack/settings',{method:'POST',body:JSON.stringify({entries,approval:document.getElementById('slack-approval').value})}));for(const k of SLACK_KEYS)document.getElementById('slack-'+k).value='';await refresh()}catch(e){show('slack-result',e.payload||e.message)}}
async function loadManifest(){try{const m=await api('/slack/manifest');lastManifestText=m.text||JSON.stringify(m.manifest,null,2)+'\\n';document.getElementById('manifest-text').value=lastManifestText;document.getElementById('manifest-url').textContent='request_url: '+m.requestUrl;show('manifest-result',{renderer:m.renderer,eventsPath:m.eventsPath})}catch(e){show('manifest-result',e.payload||e.message)}}
async function copyManifest(){if(!lastManifestText)await loadManifest();await navigator.clipboard.writeText(lastManifestText);show('manifest-result','Copied manifest JSON to clipboard.')}
async function downloadManifest(){if(!lastManifestText)await loadManifest();const blob=new Blob([lastManifestText],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='codex-chat.slack.manifest.json';a.click();URL.revokeObjectURL(url)}
async function runOperation(){try{show('op-result',await api('/codex-chat/operation',{method:'POST',body:JSON.stringify({operation:document.getElementById('op').value,approval:document.getElementById('op-approval').value})}));await refresh()}catch(e){show('op-result',e.payload||e.message)}}loadClerk().catch(()=>{});refresh();</script></body></html>`;
}
