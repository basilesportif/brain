...
- [~] Step 4 — Frontend: React/Vite app at `/admin-v2` — slice 4a DONE (committed; Opus-implemented, Fable-reviewed: 10 confirmed findings + legacy env-guard finding fixed — env-write server gate returns accepting legacy phrase OR confirmed:true, OpenRouter form seeds from server current values with old→new confirm diffs, confirmation pins read state, server-exposed confirmationKeys replace client mirrors, index.html always injected + no-store, 503 page path leak removed, symlink realpath containment, per-key writable flag, 403 taxonomy split, status poll stops on auth errors; 63/63 tests, UI + root builds green). Slice 4b IN FLIGHT 2026-07-05, FIX ROUND: implementation on disk (uncommitted; 63/63 web tests, UI build, e2e smoke green). Fable review (wf_2a52ce80-d54) confirmed 10 findings, headlined by an unsound client-side revoke design: grant-id resolution via /capabilities/check misses secondary-subject and selector-scoped grants (silent no-op revokes on the live store), single-capability revoke deletes group/bundle grants, merged per-grant previews understate impact, no atomicity/store pinning; plus preview-failure commit enablement, dismissable mid-commit dialogs, unconfirmed Reconfigure write, restart handshake POST on mount, and a REAL EMAIL in e2e (must become smoke@example.com before commit). Fix = backend grant exposure in GET /users + atomic batch revoke + storeHash pinning (full report /tmp/claude-1000/-home-tim/77eaeb13-5b01-47ab-a93c-0113ec69c5b0/tasks/wu380j3bc.output; fix agent .../tasks/a2c729c2b0ea72bce.output). Recovery: dirty tree = 4b + fixes; verify all builds/tests/e2e, re-review fix hunks, privacy grep (incl. emails), then commit.
