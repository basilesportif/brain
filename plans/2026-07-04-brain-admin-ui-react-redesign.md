...
- [~] Step 3 — Brain backend: §6.5 writes (store migration + backup, grant/identity mutations, impact preview, audit events) — IN FLIGHT 2026-07-05, FIX ROUND: implementation complete on disk (uncommitted; 51/51 tests were green). Fable's code review (Workflow wf_bd7972fe-410, 31 agents) returned 10 CONFIRMED findings; full report at /tmp/claude-1000/-home-tim/77eaeb13-5b01-47ab-a93c-0113ec69c5b0/tasks/w20w62j8z.output. Headline: migration deletes system:codex-chat-runtime which holds 2 ACTIVE ENFORCING live grants (subagents.result.deliver, system.callback.enqueue) — must be retained; /0{8}/ placeholder regex substring-matches real ids; migration bypasses self-lockout rail; empty-selector grants never match the live enforcer; suspended-subject admins counted; hash-check race; audit-append 500 after successful commit; unknown capability ids accepted; malformed body → 500 not 400; double-revoke corrupts history. Opus fix round dispatched (agent output /tmp/claude-1000/-home-tim/77eaeb13-5b01-47ab-a93c-0113ec69c5b0/tasks/ad7d24fc05a22ac98.output). Recovery: if session dies, the dirty tree is implementation+fixes; verify with `pnpm --filter @brain/web build && pnpm --filter @brain/web test`, re-review the fix hunks per finding, then commit. Live store /home/tim/.brain must never be touched by tests.
