...
- [x] Step 6 — Verify: compressed per Tim's direction 2026-07-06 — /admin-v2 deployed live, Tim exercised the console directly; formal per-workflow verification folded into Tim's hands-on use + the e2e smoke
- [x] Step 7 — Cut over `/admin` to the new app — DONE 2026-07-06 (`7d163c7`, combined with step 8; Tim waived the one-release legacy fallback). SPA at config.routePath (BRAIN_ADMIN_ROUTE_PATH, default /admin), /admin-v2 308-redirects all methods
- [x] Step 8 — Remove old console, canary store/endpoints, stale placeholder types — DONE 2026-07-06 (`7d163c7`). admin-page.ts, capabilities.ts placeholder module, canary store/endpoints, legacy sign-in smoke deleted. Startup store init is fail-open for the service / fail-closed for enforcement: corrupt store boots + 503s, ENOENT seeds minimal store via canonical writer (first allowlisted admin, enforcing), normalization idempotent + audited, zero-admins-before lockout exemption. 9-finding review fixed. NEXT (approved): capability registry — codex-chat-owned declarative registry + IPC get_capability_registry, Brain catalog from it, selector-scoped grants (see session task #9)
