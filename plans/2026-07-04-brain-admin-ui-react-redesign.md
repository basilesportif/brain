...
- [~] Step 4 — Frontend: React/Vite app at `/admin-v2` — IN FLIGHT 2026-07-05, slice 4a (scaffold + Clerk migration + serving + Home + Settings incl. server-side approval-phrase removal) dispatched to Opus; slice 4b (Setup, Users, Operations + Playwright smoke) follows. Recovery: dirty tree = Opus output; review cycle per Execution model, verify `pnpm --filter @brain/web build && pnpm --filter @brain/web test` (61 baseline) + UI build; commit per slice after Fable review.
