...
- **Behavior prompt pack**: every runtime turn now includes routing rules, skill-read requirements, directive syntax, assistant-agent-logic skill references, file-save privacy rules, generated-image routing, scratch-page/codex-chat-web routing, loop/monitor guidance, setup/migration boundaries, and active subagent steering instructions.
...
- **Todos**: `todo-add.js`, `todo-list.js`, and `todo-delete.js` are native Brain workspace commands. Natural-language todo intent, post-mutation display, and delete-reference semantics belong to the assistant-agent-logic todo skill and workspace overlays, not Brain service regexes.
