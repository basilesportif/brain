...
- **Behavior prompt pack**: every runtime turn now includes routing rules, skill-read requirements, directive syntax, todo post-mutation rules, file-save privacy rules, generated-image routing, scratch-page/codex-chat-web routing, conference-list routing, loop/monitor guidance, setup/migration boundaries, and active subagent steering instructions.
- **Main loop vs subagent routing**: prompts allow deterministic todo/project/reminder/CRM/file-save/conference-list operations to stay in the main loop when they are explicit script calls, and require subagents for repo inspection, code/docs changes, debugging, research, calendar/email/Gmail/finance/health/betting live account reads, generated images, and scratch web pages. Straightforward calendar event creation with all details supplied should dispatch at medium effort; lookup/research/ambiguous scheduling escalates.
...
- **Projects/resources/tasks**: native `project-*.js` commands cover add/list/view/update/delete, notes, resources, and project-scoped tasks. Project-note navigation should use `projects.md` and `project-notes-list.js` metadata/canonical indexes before opening full note bodies.
...
- **Conference lists/favorites**: `conference-favorite.js` updates durable `data/conference-lists/**` records and manifests with atomic locked writes; generated conference maps should be republished from durable list data rather than storing decisions only in scratch pages.
...
- **Scratch pages / codex-chat-web**: prompts require the generated-web-page skill, repo-registry authority resolution, artifact-directory builds, static validation, publisher-only deployment to unlisted `/pages/<id>/` URLs derived from the configured publisher public base URL, manifest verification, and TTL/pruning or promotion reporting.
