...
- Telegram ingress sends an immediate best-effort `👀` reaction for authorized
  ...
  dispatch. This mirrors `codex-chat` service-level acknowledgement and does
  not depend on provider-emitted `react` directives; duplicate `👀` reactions
  for the same Telegram message are skipped if a later outbound action asks for
  one.
